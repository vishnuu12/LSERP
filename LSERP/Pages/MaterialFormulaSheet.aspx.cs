    ...
    protected void ddlMaterialName_SelectIndexChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            if (ddlMaterialName.SelectedIndex > 0)
            {
                ...
                ddlMaterialSpecsName.DataSource = ds.Tables[0];
        ...
        try
        {
            if (ValidateControls())
            {
                ...
                else if (ds.Tables[0].Rows[0]["Message"].ToString() == "Updated")
                {
                    ...
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Save", "ErrorMessage('Error','You Entered Formula Is Wrong');hideLoader();", true);
