...
public partial class Annexure_SOW_points : System.Web.UI.Page
{
    #region "Declaration"
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                int SOWPID = Convert.ToInt32(arg);
                DataSet ds = _objSales.deleteSOWPoints(SOWPID);
                if (ds.Tables[0].Rows[0]["Message"] == "Deleted")
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "SuccessMessage('Success','SOW Point Deleted successfully');hideLoader();", true);
                }
                else if (ds.Tables[0].Rows[0]["Message"] == "AU")
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "ErrorMessage('Error','SOW Point Already in use. Cannot be deleted');hideLoader();", true);
                }
    ...
    protected void gvAnnexure_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName.ToString() == "ViewAnnexure")
            {
                ...
                string SOWPID = gvAnnexure.DataKeys[index].Values[0].ToString();
                string SOWID = ((Label)gvAnnexure.Rows[index].FindControl("lblSOWID")).Text;
                txtPoint.Text = ((Label)gvAnnexure.Rows[index].FindControl("lblPoint")).Text;
                _objSales.GetSOWList(ddlSOW);
                ddlSOW.ClearSelection(); //making sure the previous selection has been cleared
                ddlSOW.Items.FindByValue(SOWID).Selected = true;
                hdnSOWPID.Value = SOWPID;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                _objSales.SOWPID = Convert.ToInt32(hdnSOWPID.Value);
                _objSales.SOWID = Convert.ToInt32(ddlSOW.SelectedValue);
                _objSales.Points = txtPoint.Text.Trim();
                DataSet ds = _objSales.saveSOWPoints();
                ...
                if (ds.Tables[0].Rows[0]["Message"] == "AE")
                {
                    ...
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "ErrorMessage('Error','SOW Header Already Exists'); ShowPopup();showDataTable();", true);
                ...
                else
                {
                    ...
                    if (Convert.ToInt32(hdnSOWPID.Value) == 0)
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "SuccessMessage('Success','SOW Header Saved successfully');hideLoader();", true);
                    else
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "SuccessMessage('Success','SOW Header Updated successfully');hideLoader();", true);
    ...
    protected void btnExcelDownload_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            _objc.SaveExcelFile("Annexure_SOW_Points.aspx", LetterName, _objSession.employeeid);
    ...
    protected void btnPDFDownload_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            _objc.SavePDFFile("Annexure_SOW_Points.aspx", pdffile, _objSession.employeeid);
    ...
    private void bindAnnexure()
    {
        ...
        try
        {
            dsAnnexure = _objSales.getSOWPoints();
