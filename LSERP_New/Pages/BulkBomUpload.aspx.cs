...
public partial class Pages_BulkBomUpload : System.Web.UI.Page
{
    #region"Declaration"
    ...
    protected void btnBomBulkUpload_Click(object sender, EventArgs e)
    {
        ...
        {
            string strFile = "";
            if (fpbombulk.HasFile)
            {
                AttachmentName = Path.GetFileName(fpbombulk.PostedFile.FileName);
                AttachmentName = AttachmentName.Replace("/", "");
                string[] extension = AttachmentName.Split('.');
                if (extension[1].ToString().ToUpper() == "XLSX" || extension[1].ToString().ToUpper() == "XLS")
                {
                    AttachmentName = extension[0] + '.' + extension[1];
                    string path = LetterPath + AttachmentName;
                    ...
                    fpbombulk.SaveAs(path);

                    // string httppath = Path.Combine(ExcelPath, AttachmentName);
                    // dt = objc.ExcelToDataTable(httppath);
                    dt = objc.ExcelToDataTable(path);
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        {
                            con.Open();
                            string deleteQuery = "truncate table dbo.LSE_BomBulkUpload"; // just delete them all
                            SqlCommand sqlComm = new SqlCommand(deleteQuery, con);
                            ...
                            using (SqlBulkCopy sqlBulkCopy = new SqlBulkCopy(con))
                            {
                                ...
                                sqlBulkCopy.DestinationTableName = "dbo.LSE_BomBulkUpload";

                                //[OPTIONAL]: Map the DataTable columns with that of the database table
                                sqlBulkCopy.ColumnMappings.Add("PartNo", "PartNo");
                                sqlBulkCopy.ColumnMappings.Add("PartName", "PartName");
                                sqlBulkCopy.ColumnMappings.Add("AddtionalPart", "AddtionalPart");
                                sqlBulkCopy.ColumnMappings.Add("Remarks", "Remarks");
                                sqlBulkCopy.ColumnMappings.Add("ID", "ID");
                                sqlBulkCopy.ColumnMappings.Add("DEP", "DEP");
                                sqlBulkCopy.ColumnMappings.Add("PIT", "PIT");
                                sqlBulkCopy.ColumnMappings.Add("THK", "THK");
                                sqlBulkCopy.ColumnMappings.Add("NOP", "NOP");
                                sqlBulkCopy.ColumnMappings.Add("NOC", "NOC");
                                sqlBulkCopy.ColumnMappings.Add("LOC", "LOC");
                                sqlBulkCopy.ColumnMappings.Add("MDIA", "MDIA");
                                sqlBulkCopy.ColumnMappings.Add("OD", "OD");
                                sqlBulkCopy.ColumnMappings.Add("TTP", "TTP");
                                sqlBulkCopy.ColumnMappings.Add("SWT", "SWT");
                                sqlBulkCopy.ColumnMappings.Add("LEN", "LEN");
                                sqlBulkCopy.ColumnMappings.Add("WID", "WID");
                                sqlBulkCopy.ColumnMappings.Add("CTHK", "CTHK");
                                sqlBulkCopy.ColumnMappings.Add("QTY", "QTY");
                                sqlBulkCopy.ColumnMappings.Add("AWT", "AWT");
                                sqlBulkCopy.ColumnMappings.Add("Cl", "Cl");
                                sqlBulkCopy.ColumnMappings.Add("CW", "CW");
                                sqlBulkCopy.ColumnMappings.Add("OTHK", "OTHK");
                                sqlBulkCopy.ColumnMappings.Add("TQTY", "TQTY");
                                sqlBulkCopy.ColumnMappings.Add("WT", "WT");
                                sqlBulkCopy.ColumnMappings.Add("MOC", "MOC");
                                sqlBulkCopy.ColumnMappings.Add("MRATE", "MRATE");
                                sqlBulkCopy.ColumnMappings.Add("MCOST", "MCOST");
                                sqlBulkCopy.ColumnMappings.Add("LRATE", "LRATE");
                                sqlBulkCopy.ColumnMappings.Add("LCOST", "LCOST");
                                sqlBulkCopy.ColumnMappings.Add("LS", "LS");
                                sqlBulkCopy.ColumnMappings.Add("SS", "SS");
                                sqlBulkCopy.ColumnMappings.Add("Side", "Side");
                                sqlBulkCopy.ColumnMappings.Add("MCRATE", "MCRATE");
                        ...
                        objDesign.EDID = Convert.ToInt32(ddlItemName.SelectedValue);
                        objDesign.VersionNumber = Convert.ToInt32(ddlVersionNumber.SelectedItem.Text);
                        ds = objDesign.SaveBomCostBulkDetails();

                        if (ds.Tables[0].Rows.Count > 0)
                        {
                            if (ds.Tables[0].Rows[0]["Message"].ToString() == "InValidRecord")
                                ReportExcelDownload(ds.Tables[0], objSession.employeeid);
                            else if (ds.Tables[0].Rows[0]["Message"].ToString() == "Added")
                                ScriptManager.RegisterStartupScript(this, this.GetType(), "table", "SuccessMessage('Success','Bom Bulk Details Inserted Successfully');", true);
                            else
                                ScriptManager.RegisterStartupScript(this, this.GetType(), "table", "InfoMessage('Information','" + ds.Tables[0].Rows[0]["Message"].ToString() + "');", true);
                        }
                    }
                    else
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "table", "ErrorMessage('Error','File Not Found');", true);
                }
                else
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "table", "ErrorMessage('Error','File Format Invalid');", true);
            }
            else
                ScriptManager.RegisterStartupScript(this, this.GetType(), "table", "ErrorMessage('Error','Upload File');", true);
        }
        catch (Exception ex)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "table", "ErrorMessage('Error','Error Occured');", true);
            Log.Message(ex.ToString());
        }
    }
    ...
    public void ReportExcelDownload(DataTable dt, string UserID)
    {
        ...
        try
        {
            ...
            if (File.Exists(strFile))
                File.Delete(strFile);
            objc.exportExcel(dt, rowcount, ColumnCount, strFile, LetterName, "InValid Details", "", 2, GemBoxKey);
            objc.SaveExcelFile(pagename[pagename.Length - 1], LetterName, UserID);
        }
        catch (Exception ex)
        {
            Log.Message(ex.ToString());
