...
public partial class Annexure_SOW : System.Web.UI.Page
{
    #region "Declaration"
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                int SOWID = Convert.ToInt32(arg);
                DataSet ds = _objSales.deleteAnnexure(SOWID);
                if (ds.Tables[0].Rows[0]["Message"] == "Deleted")
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "SuccessMessage('Success','SOW Header Deleted successfully');hideLoader();", true);
                }
                else if (ds.Tables[0].Rows[0]["Message"] == "AU")
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "ErrorMessage('Error','SOW Header Already in use. Cannot be deleted');hideLoader();", true);
                }
    ...
    protected void gvAnnexure_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            if (e.CommandName.ToString() == "ViewAnnexure")
            {
                ...
                string SOWID = gvAnnexure.DataKeys[index].Values[0].ToString();
                txtHeaderName.Text = ((Label)gvAnnexure.Rows[index].FindControl("lblAnnexure")).Text;
                hdnSOWID.Value = SOWID;
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                _objSales.SOWID = Convert.ToInt32(hdnSOWID.Value);
                _objSales.AnnexureName = txtHeaderName.Text.Trim();
                ...
                else
                {
                    ...
                    if (Convert.ToInt32(hdnSOWID.Value) == 0)
    ...
    protected void btnExcelDownload_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            _objc.SaveExcelFile("Annexure_SOW.aspx", LetterName, _objSession.employeeid);
    ...
    protected void btnPDFDownload_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            _objc.SaveHtmlFile(URL,"Annexure ScopeOf Work",lbltitle.Text,div);

            _objc.GenerateAndSavePDF(LetterPath, strFile, pdffile, URL);

            _objc.SavePDFFile("Annexure_SOW.aspx", pdffile, _objSession.employeeid);
