...
public partial class Pages_MaterialPlanning : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (target == "ViewCFDocs")
                ViewCertificatesDocs(arg.ToString());
        }
        catch (Exception ex)
        {
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            if (objc.Validate(divInput))
            {
				...
				objMat.AWTValue = Convert.ToDecimal(txtRequiredActualWeight.Text);
    ...
    protected void gvMaterialPlanningDetails_OnRowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        try
        {
            ...
            if (e.CommandName == "addPMP")
            {
				...
				//BindAWTValueForPartMaster();
                //string AWTValue = "BOM AWT Value : " + ViewState["MSMIDValue"].ToString();
                //lblAWTValue.Text = AWTValue.ToString();
                ...
                if (ds.Tables[6].Rows[0]["JobCardStatus"].ToString() == "InComplete")
                {
                    ...
                    {
                        txtRequiredActualWeight.Text = ds.Tables[7].Rows[0]["AWTValue"].ToString();
                    }
                    ...
                    if (ds.Tables[8].Rows.Count > 0)
                        lblAWTValue.Text = ds.Tables[8].Rows[0]["MSMIDValue"].ToString();
                    else
                        lblAWTValue.Text = "0.00";
                    ...
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "PopUP", "InfoMessage('Information','" + ds.Tables[6].Rows[0]["JobCardStatus"].ToString() + "')", true);
            }
    ...
    protected void gvMaterialPlanningDetails_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                LinkButton btnviewbomdrawing = (LinkButton)e.Row.FindControl("btnviewbomdrawing");
                ...
                if (dr["PartType"].ToString() == "C" || dr["PartType"].ToString() == "O")
                {
                    btnAdd.Visible = false;
                    btnAddAttach.Visible = false;
                }

                if (string.IsNullOrEmpty(dr["BomPartDrawing"].ToString()))
                    btnviewbomdrawing.Visible = false;
                else
                    btnviewbomdrawing.Visible = true;
            }
        }
        catch (Exception ex)
        {
    ...
    protected void gvMPItemDetails_OnRowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (dr["QPStatus"].ToString() == "InComplete")
                    btnAdd.Visible = false;
                else
                    btnAdd.Visible = true;

                e.Row.Cells[7].Attributes.Add("style", "word-break: break-word;width: 10%;");
            }
        }
        catch (Exception ex)
        {
	...
	private void BindAWTValueForPartMaster()
    {
        DataSet ds = new DataSet();
        objP = new cProduction();
        try
        {
            ...
            ds = objP.GetAWTValueForPartMasterByBOMID();
            ViewState["MSMIDValue"] = ds.Tables[0].Rows[0]["MSMIDValue"].ToString();
    ...
    private void ViewCertificatesDocs(string Filepath)
    {
        DataSet ds = new DataSet();
        try
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Message", "opennewtab('" + Filepath.ToString() + "')", true);
        }
        catch (Exception ex)
        {
            Log.Message(ex.ToString());
    ...
    private void ddlCustomernameAndRFPNoLoad()
    {
        ...
        try
        {
            ...
            dsCustomer = objP.getRFPCustomerNameByMaterialPlanning(Convert.ToInt32(objSession.employeeid), ddlCustomerName, rblRFPChange.SelectedValue);
            dsRFPHID = objP.GetRFPNoDetaisByMaterialPlanning(Convert.ToInt32(objSession.employeeid), ddlRFPNo, rblRFPChange.SelectedValue);
    ...
    private void BindItemDetails()
    {
        ...
        try
        {
            ...
            ds = objMat.GetItemDetailsByRFPHIDByMaterialPlanning(null, "LS_GetItemDetailsByRFPHIDInMaterialPlanning");
