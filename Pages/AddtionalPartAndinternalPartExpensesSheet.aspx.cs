...
public partial class Pages_AddtionalPartAndinternalPartExpensesSheet : System.Web.UI.Page
{
    #region "Declaration"
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                HttpContext.Current.Session["APIP"] = null;
    ...
    public static object GetData()
    {
        ...
        try
        {
            ...
            string PartType = HttpContext.Current.Request.Params["PartType"];

            if (HttpContext.Current.Session["FromDate"] != FromDate ||
                HttpContext.Current.Session["ToDate"] != ToDate)
            {
                data = GetAddtionalPartExpensesDetails(FromDate, ToDate, PartType);
                HttpContext.Current.Session["APIP"] = data;
                HttpContext.Current.Session["FromDate"] = FromDate;
                HttpContext.Current.Session["ToDate"] = ToDate;
                HttpContext.Current.Session["PartType"] = PartType;
            }
            else
                data = (List<Dictionary<string, object>>)HttpContext.Current.Session["APIP"];
              ...
              !string.IsNullOrWhiteSpace(search))
            {
                data = data.Where(p =>
                           ...
                           || p["MaterialName"].ToString().ToLower().Contains(search.ToLower())
                           || p["ISSUEDWEIGHT"].ToString().ToLower().Contains(search.ToLower())
                           || p["UnitQuoteCost"].ToString().ToLower().Contains(search.ToLower())
                           || p["TotalCost"].ToString().ToLower().Contains(search.ToLower())
                          ).ToList();
            }
    ...
    [WebMethod]
    [System.Web.Script.Services.ScriptMethod(ResponseFormat = ResponseFormat.Json, UseHttpGet = true)]
    public static List<Dictionary<string, object>> GetAddtionalPartExpensesDetails(string FromDate, string ToDate, string PartType)
    {
        DataSet ds = new DataSet();
        ...
        try
        {
            ...
            objRpt.PartType = PartType;

            ds = objRpt.GetAddtionalPartExpensesDetailsByFromdateandtodate();
