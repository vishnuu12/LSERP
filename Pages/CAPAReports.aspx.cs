...
public partial class Pages_CAPAReports : System.Web.UI.Page
{
    #region "Declaration"
    ...
    protected void btnExcel_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string strFile = "";
            string LetterName = "CAPAReports" + ".xls";
            ...
            exportExcel(ds, rowcount, ColumnCount, strFile, LetterName, "", "CAPA Reports", 2, GemBoxKey);
        }
        catch (Exception ex)
        {
            Log.Message(ex.ToString());
    ...
    private void BindCAPAReports()
    {
        ...
        try
        {
            ...
            ds = objDesign.GetCAPAReports();

            if (ds.Tables[0].Rows.Count > 0)
            {
                ViewState["DesignKPIReports"] = ds.Tables[0];
                gvCAPAReports.DataSource = ds.Tables[0];
                gvCAPAReports.DataBind();
            }
            else
            {
                gvCAPAReports.DataSource = "";
                gvCAPAReports.DataBind();
            }
    ...
    public void exportExcel(DataTable dt, int rowCount, int columnCount, string fileName, string LetterName, string schoolName, string reportName, int startRow, string gemBoxKey)
    {
        try
        {
            if ((dt.Rows.Count > 0) && (rowCount != 0) && (columnCount != 0))
            {
                ...
                var worksheet = workbook.Worksheets.Add("CAPA Reports");
