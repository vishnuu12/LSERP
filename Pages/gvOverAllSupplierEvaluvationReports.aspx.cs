...
public partial class Pages_gvOverAllSupplierEvaluvationReports : System.Web.UI.Page
{
    #region"DecLaration"
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            BindSupplierEvaluvationReports();
        }
        catch (Exception ex)
        {
            Log.Message(ex.ToString());
    ...
    private void BindSupplierEvaluvationReports()
    {
        objR = new cReports();
        ...
        try
        {
            ...
            ds = objR.GetSupplierEvaluvationReportDetailsByFormDateAndTodate();

            ViewState["DataTable"] = ds.Tables[0];

            if (ds.Tables[0].Rows.Count > 0)
            {
                gvOverAllSupplierEvaluvationReports.DataSource = ds.Tables[0];
                gvOverAllSupplierEvaluvationReports.DataBind();
            }
            else
            {
                gvOverAllSupplierEvaluvationReports.DataSource = "";
                gvOverAllSupplierEvaluvationReports.DataBind();
            }
        }
        catch (Exception ex)
    ...
    {
        if (gvOverAllSupplierEvaluvationReports.Rows.Count > 0)
        {
            gvOverAllSupplierEvaluvationReports.UseAccessibleHeader = true;
            gvOverAllSupplierEvaluvationReports.HeaderRow.TableSection = TableRowSection.TableHeader;
        }
    }
