...
public partial class Pages_POApproval : System.Web.UI.Page
{
    #region"Declaration"
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        try
        {
            if (IsPostBack == false)
            {
                ...
                BindPOApprovedValues();

                bindPOApprovedDetails();
            }
            if (target == "PrintPO")
                ...
                if (chk.Checked)
                {
                    ...
                    if (CommandName.ToString() == "Approve")
                    {
                        if (objSession.type == 1)
                            objPc.POApprovalStatus = 1;
                        else if (objSession.type == 9)
                            objPc.POApprovalStatus = 6;
                        else
                            objPc.POApprovalStatus = 8;
                    }
    ...
    private void bindPurchaseOrderPDFDetails(int SPOID)
    {
        DataSet ds = new DataSet();
        objPc = new cPurchase();
        string Tax = "";
    ...
    private void BindPOApprovedValues()
    {
        DataSet ds = new DataSet();
        objPc = new cPurchase();
        try
        {
            ds = objPc.GetPOApprovedValues();

            lblPOApprovedLMV.Text = ds.Tables[0].Rows[0]["LMPOvalue"].ToString();
            lblPOApprovedCMV.Text = ds.Tables[0].Rows[0]["CMPOValue"].ToString();
            lblPOpendingCMV.Text = ds.Tables[0].Rows[0]["POPendingValue"].ToString();
            lblTotalAmtCMV.Text = "<span style='color:#000;font-weight:bold;'>" + "'Total Amt :'" + ds.Tables[0].Rows[0]["Totalvalue"].ToString() + "</span>";
        }
        catch (Exception ex)
        {
            Log.Message(ex.ToString());
    ...
    private void bindPOApprovedDetails()
    {
        DataSet ds = new DataSet();
        objPc = new cPurchase();
        try
        {
            ds = objPc.GetPoApprovedDetails();

            if (ds.Tables[0].Rows.Count > 0)
            {
                gvPOApprovedDetails.DataSource = ds.Tables[0];
                gvPOApprovedDetails.DataBind();
            }
            else
            {
                gvPOApprovedDetails.DataSource = "";
                gvPOApprovedDetails.DataBind();
            }

            if (gvPOApprovedDetails.Rows.Count > 0)
                gvPOApprovedDetails.HeaderRow.TableSection = TableRowSection.TableHeader;
            ScriptManager.RegisterStartupScript(this, this.GetType(), "datatble", "ShowDatatable();", true);
        }
        catch (Exception ex)
        {
            Log.Message(ex.ToString());
    ...
    #region"PageLoadCompleted Events"
