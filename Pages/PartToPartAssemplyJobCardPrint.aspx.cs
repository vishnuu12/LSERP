...
public partial class Pages_PartToPartAssemplyJobCardPrint : System.Web.UI.Page
{
    #region"Declaretion"
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ...
                ViewState["PAPDID"] = Request.QueryString["PAPDID"].ToString();
                JobCardPrintDetails();
            }
        }
        catch (Exception ex)
    ...
    private void JobCardPrintDetails()
    {
        ...
        try
        {
            ...
            objP.PAPDID = Convert.ToInt32(ViewState["PAPDID"].ToString());

            ds = objP.GetPartTopartAssemplyJobCardPrintDetailsByJCHIDAndPAPDID();

            lblProcessName_FW_P_H.Text = ds.Tables[0].Rows[0]["RFPNo"].ToString()+" / "+ds.Tables[0].Rows[0]["ProcessName"].ToString();
            ...
            lblPartname_FW_P.Text = ds.Tables[1].Rows[0]["PartName"].ToString() + " To " + ds.Tables[1].Rows[1]["PartName"].ToString();

            lblMaterialCategory_FW_P.Text = ds.Tables[1].Rows[0]["CategoryName"].ToString() + " - " + ds.Tables[1].Rows[1]["CategoryName"].ToString();
            lblMaterialGrade_FW_P.Text = ds.Tables[1].Rows[0]["GradeName"].ToString() + " - " + ds.Tables[1].Rows[1]["GradeName"].ToString();
            lblThickness_FW_P.Text = ds.Tables[1].Rows[0]["THKValue"].ToString() + " - " + ds.Tables[1].Rows[1]["THKValue"].ToString();
            lblMRNNo_FW_P.Text = ds.Tables[1].Rows[0]["MRNNo"].ToString() + " To " + ds.Tables[1].Rows[1]["MRNNo"].ToString();
    ...
    {
        if (gvBeforeWelding_FW_P.Rows.Count > 0)
            gvBeforeWelding_FW_P.HeaderRow.TableSection = TableRowSection.TableHeader;
        if (gvDuringWelding_FW_P.Rows.Count > 0)
            gvDuringWelding_FW_P.HeaderRow.TableSection = TableRowSection.TableHeader;
        if (gvFinalWelding_FW_P.Rows.Count > 0)
            gvFinalWelding_FW_P.HeaderRow.TableSection = TableRowSection.TableHeader;
    }
