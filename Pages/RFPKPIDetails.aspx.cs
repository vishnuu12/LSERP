...
public partial class Pages_RFPKPIDetails : System.Web.UI.Page
{
    #region"DecLaration"
    ...
    protected void gvMonthlyOrderPODetails_OnRowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            string RFPHID = gvMonthlyOrderPODetails.DataKeys[index].Values[0].ToString();
            ...
            string Page = url.Replace(Replacevalue, "RFPKPIItemDetails.aspx?RFPHID=" + RFPHID);
    ...
    protected void btnExcelDownload_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            dt = (DataTable)ViewState["RFPKPIReleasedDetails"];
            ...
            string LetterName = "RFP RELEASED DETAILS " + "-" + ViewState["Month"].ToString() + "-" + ViewState["Year"].ToString() + "-" + MAXEXID + ".xls";
    ...
    private void BindRFPKPIDetails()
    {
        ...
        try
        {
            ...
            ds = objR.GetRFPKPIDetailsByMonthAndYear();

            if (ds.Tables[0].Rows.Count > 0)
            {
                ViewState["RFPKPIReleasedDetails"] = ds.Tables[0];
    ...
    public void exportExcel(DataTable dt, string fileName, string LetterName, string gemBoxKey)
    {
        try
        {
            if ((dt.Rows.Count > 0))
            {
                ...
                SheetLocation1 = workbook.Worksheets.Add("RFP RELEASED DETAILS");

                if (dt.Rows.Count > 0)
                {
                    SheetLocation1 = exportex(dt, dt.Rows.Count, dt.Columns.Count, LetterName,
                        " RFP RELEASED " + ViewState["Month"].ToString() + " / " + ViewState["Year"].ToString() + "", "", 2, SheetLocation1);
                }
